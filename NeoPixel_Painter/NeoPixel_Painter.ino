...
#define N_LEDS       64 // Max value is 170 (fits one SD card block)
#define CARD_SELECT   10 // SD card select pin (some shields use #4, not 10)
#define LED_PIN        6 // NeoPixels connect here
#define SPEED         100 // Speed-setting dial
#define BRIGHTNESS    1000 // Brightness-setting dial
#define TRIGGER       LOW // HIGH setup / display
...
void setup() {
  ...
  startupTrigger = TRIGGER; // Poll startup trigger ASAP
  Serial.begin(57600);
  ...
  if(startupTrigger == HIGH) { // No button press
    ...
    minBrightness = map(BRIGHTNESS, 0, 1023, 1, minBrightness);
    ...
    while(TRIGGER == LOW); // Wait for button release
...
void loop() {
  ...
  while(TRIGGER == HIGH);   // Wait for trigger button
  ...
  uint32_t linesPerSec = map(SPEED, 0, 1023, 10, maxLPS);
  ...
  for(;;) {
    ...
    if(++block >= nBlocks) {                   // Past last block?
      if(TRIGGER == HIGH) {       // Trigger released?
